...
export function Renderer(canvas)
{
    ...
    this.init = function()
    {
        _ctx = _canvas.getContext("webgl", { alpha: false });
     ...
     * @param {boolean} hasDepth Should a depth buffer stencil be attached to the render target? Default = false
     * @return {RenderTarget} A render target bound to a context for use, with a respective texture
     */
    this.createRenderTarget = function(width, height, type, format, numAttachments, hasDepth)
    {
        numAttachments = numAttachments === undefined ? 1 : numAttachments;
        hasDepth = hasDepth === undefined ? false : hasDepth;

        return CreateBoundRenderTarget(_ctx, width, height, type, format, numAttachments, hasDepth);
    }
    //--------------------------------------------------------------------------------------------
    ...
    this.fullScreenPass = function(target, camera, textures, effect, techniqueName, passName)
    {
        ...
        _fullScreenTriangle.apply(_ctx);
        _ctx.drawElements(
            _ctx.TRIANGLES,
            3,
            _ctx.UNSIGNED_SHORT,
