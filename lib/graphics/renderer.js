...
export function Renderer(canvas)
{
     ...
     */
    this.draw = function(transform, mesh, effect, techniqueName, passName)
    {
        if (transform === null)
        {
        ...
        var fieldOfView = 90.0 * Math.PI / 180.0;
        var aspect = _ctx.canvas.clientWidth / _ctx.canvas.clientHeight;
        var zNear = 0.1;
        var zFar = 1000.0;
        var projectionMatrix = glm.Matrix4x4.create();

        glm.Matrix4x4.perspective(projectionMatrix,
                        fieldOfView,
                        aspect,
                        zNear,
                        zFar);

        var viewMatrix = glm.Matrix4x4.create();
        ...
        if (matrixLocation !== null)
        {
            _ctx.uniformMatrix4fv(matrixLocation.offset, false, transform.getLocalToWorld());
        }
